...
{		
        ...
        if(uv.y < 0.5)
        {
		    if(uv.x < 0.5)
            {
                gl_FragColor = vec4( magma(current.r), 1.0);
            }
            else
            {
                gl_FragColor = vec4( viridis(current.r), 1.0);
            }
        }
        else
        {
            if(uv.x < 0.5)
            {
                gl_FragColor = vec4( plasma(current.r), 1.0);
            }
            else
            {
                gl_FragColor = vec4( vec3(current.r), 1.0);
            }
        }
    }
`;